...
class MainWidget(GridLayout):
    def __init__(self, **kwargs):
        ...
        for i in range(0, 8):
            ...
            object_name = Image(source='default.jpg')
            left_grid_2_1.add_widget(object_name)
            input_object_z = TextInput(text='0', multiline=False, size_hint=(.2, .2))
            left_grid_2_1.add_widget(input_object_z)
            btn_grab_obj = Button(text="Go", font_size=16, size_hint=(.30, .30))
            btn_grab_obj.bind(on_press=self.move_object)
        ...
        if (self.inf_coordinate_robot_final == "Done" and self.inf_coordinate_marker_center_final == "Done"):
            self.status_marker_coordinate_final = Label(text ="Done")
        else:
            self.status_marker_coordinate_final = Label(text ="---")
        right_grid_1.add_widget(self.status_marker_coordinate_final)
    ...
    def load_video_calibration(self, *args):
        ...
        self.image_frame_calibration = self.image_frame.copy()
        buffer = cv2.flip(frame, 0).tostring()
        texture = Texture.create(size=(frame.shape[1], frame.shape[0]), colorfmt='bgr')
        texture.blit_buffer(buffer, colorfmt='bgr', bufferfmt='ubyte')
    ...
    def save_camera_var(self, *args):
        ...
        self.save_coor_to_file(self.camera_dist, "camera_dist")
        # self.save_coor_to_file(self.coordinate_camera, "coordinate_camera")
        # self.save_coor_to_file(self.coordinate_marker_center, "coordinate_marker_center")
    ...
    def move_object(self, x, y, z, speed=30, wait=True):
        # X, Y, Z, SPEED
        self.swift.set_position(x, y, z, speed=speed, wait=wait)
        time.sleep(1)
    ...
    def attach_servo(self, *args):
        ...
        self.inf_coordinate_marker_center_final = "Done"
        self.save_coor_to_file(self.coordinate_robot_final, "coordinate_robot_final")
        self.save_coor_to_file(self.coordinate_marker_center_final, "coordinate_marker_center_final")
