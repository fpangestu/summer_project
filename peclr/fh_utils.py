...
def get_bbox_from_pose(pose, maxcoord = None):
    ...
    if maxcoord is not None:
        x1 = int(np.min(x))
        y1 = int(np.min(y))
        x2 = min(int(np.max(x)),maxcoord)
        y2 = min(int(np.max(y)),maxcoord)
    else:
        x1 = int(np.min(x))
        y1 = int(np.min(y))
        x2 = int(np.max(x))
...
def preprocess(img, K, T, crop_size):
    ...
    img = affimg.astype(np.float32) / 255
