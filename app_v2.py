...
class MainWidget(GridLayout):
    ...
    def load_video_2(self, *args):
        ...
        thresInv_adaptive = cv2.adaptiveThreshold(img_blur, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 21, 2)
        ...
        for c in contours:
            ...
            cv2.putText(frame_2, str_object, (x + w, y + h), 0, 0.3, (0, 255, 0))

            x2 = x + int(w/2)
            y2 = y + int(h/2)
            cv2.circle(frame_2, (x2, y2), 4, (0, 0, 255), -1)
            str_object = str(x2) + ", " + str(y2)
            cv2.putText(frame_2, str_object, (x2, y2 + 10), 0, 0.4, (0, 0, 255))
    ...
    def calibrate_camera_aruco(self, *args):
        ...
        if isExist:
            ...
            np.savetxt("./lastcameramtx.txt",self.camera_mtx)
            np.savetxt("./lastdist.txt",self.camera_dist)
